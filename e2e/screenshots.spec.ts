...
/**
 * README用スクリーンショット取得スクリプト (Playwright Test形式)。
 *
 * `npm run screenshots` (= `playwright test --project=screenshots`) で
 * 起動する専用project。通常のchromium project (E2E 10 件) からは
 * playwright.config.tsのtestIgnoreで除外しているので、`npm run e2e`
 * のテスト件数には含まれない。
 *
 * 取得画像: docs/screenshots/{pc,sp}-{empty,result}.pngの 4 枚
 *   - pc:  1280×800 viewport
 *   - sp:  393×852 viewport (iPhone 15 相当)
 *   - empty:  認証 + プライバシー同意済の空状態
 *   - result: サンプル「歯科」を選択 → 抽出ボタン後のSOAP 4 カード表示
 *
 * /api/authと /api/extractはroute mock。本番deployせずに撮れる
 * ようにし、Anthropic API課金も発生させない。
 */
...
/**
 * Next.js 16 dev modeが描画する右下のIssuesバッジ / dev toolsオーバーレイを
 * README用スクショから除外する。dev overlayは本番 (next build --webpack) には
 * 出ないため、製品としての見た目に含めるべきではない。
 *
 * 該当要素は `nextjs-portal` Custom Elementとしてshadow DOM経由で挿入される
 * (Next.js 16 系のdev tools実装) ので、custom element名を狙えば確実。
 */
...
async function captureFlow(page: Page, prefix: string) {
  ...
  // Next.js dev overlayを非表示にする (本番には出ない要素)
  ...
  // 結果描画後に再度dev overlayを非表示 (描画タイミングで再挿入されるケースに備える)
  await page.addStyleTag({ content: HIDE_DEV_OVERLAY_CSS });
  await page.screenshot({
...
test.describe('README用スクリーンショット', () => {
