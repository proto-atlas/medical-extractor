...
export function DocumentInput({
  ...
  // controlled selectの表示値。サンプル読込後は選択をクリアして
  ...
  async function handleFile(file: File | null) {
    ...
    try {
      // .txt / .mdはMIME型がtext/plain or text/markdown等まちまちなので
      // 拡張子で判定。ブラウザが認識しないファイルでもtext() で読めるなら受け入れる。
  ...
  function handleSampleChange(id: string) {
    ...
    // 即座にselectionをリセットして同じサンプルを再選択可能に
    setSelectedSampleId('');
  }

  function handleVoiceTranscript(chunk: string) {
    // 既存テキストへ追記。前後にスペースor改行を入れすぎず、自然な追加にする。
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between text-xs">
        <span
          className={
            // text-slate-400 はライトbg-whiteでAAを満たさないためtext-slate-500 に上げる
            overLimit
