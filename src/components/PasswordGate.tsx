...
/**
 * /api/authを呼んで結果をExtractErrorCodeに正規化する。
 *
 * 確認した認証UI課題「認証UIが 429 と 401 を区別しない」への対応。
 * booleanだと 429 (rate_limit) でも「アクセスキーが正しくありません」と表示され、
 * 利用者が誤りに気付けない。ApiErrorResponseのerror codeを返して呼び出し側で
 * labelFor() で日本語化する設計に統一する (raw errorをUIに出さない方針も継続)。
 */
async function verifyPassword(candidate: string): Promise<VerifyResult> {
  try {
    ...
    // ネットワーク失敗等。サーバ到達不能なのでupstream_unavailableに寄せる
    return { ok: false, code: 'upstream_unavailable' };
  }
}

/**
 * 401 (unauthorized) は失効キーとしてlocalStorageを消すべき。
...
export function PasswordGate({ children }: Props) {
  ...
  useEffect(() => {
    ...
    // 保存されたアクセスキーをサーバー検証してからメインUIを出す
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (result.ok) {
      ...
      // ExtractErrorCodeをERROR_LABELSの日本語に変換して表示
      // (rate_limitは「短時間に多くのリクエスト...」、unauthorizedは
      // 「アクセスキーが正しくありません」など、codeごとに別文言)
