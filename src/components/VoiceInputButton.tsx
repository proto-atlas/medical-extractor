...
interface Props {
  // 確定したテキストを受け取る (継続的にappendされる想定)
  onTranscript: (chunk: string) => void;
  disabled?: boolean;
}

/**
 * Web Speech API音声入力ボタン。
 * - ブラウザ非対応時はボタンを描画せず代わりに「非対応」テキストのみ表示。
 * - 録音中はボタン文言を切替、停止ボタンとして機能。
 * - 確定したテキスト (isFinal) のみ親に通知し、暫定 (interim) は捨てる。
 *   暫定を反映すると編集途中のカーソル位置が崩れたり、AI抽出に不安定なテキストが
 *   流れ込む可能性があるため。
 * - エラー (no-speech / aborted / not-allowed等) は内部stateに短いメッセージで保持し
 *   ボタン下に表示。
 */
export function VoiceInputButton({ onTranscript, disabled = false }: Props) {
  ...
  // ブラウザ判定はマウント後 (SSRでwindow参照不可)
  useEffect(() => {
    setAvailable(isSpeechRecognitionAvailable());
  }, []);

  // クリーンアップ: アンマウント時に走り続けないようabort
  useEffect(() => {
    return () => {
  ...
  function start() {
    ...
    rec.onresult = (event: SpeechRecognitionEvent) => {
      // event.resultsはnew + 既存全て、event.resultIndex以降が新着分。
  ...
  function stop() {
    ...
    // onendでsetIsListening(false) されるので、ここでは何もしない
  }

  if (!available) {
    return (
      <p className="text-xs text-slate-500 dark:text-slate-400">
        音声入力はこのブラウザでは利用できません (Chrome / Edge / Safariで対応)。
      </p>
    );
  }
