 ...
 * 「架空データのみ使用してください」の同意を促し、localStorageに記録する。
 * 一度同意すると以後表示されない (常時バナーはpage.tsx側で別途表示)。
 *
 * a11y:
 *   - role="dialog" / aria-modal / aria-labelledby
 *   - フォーカスをマウント直後にprimaryボタンへ
 *   - ESCで閉じない (意図的に同意ボタンクリックのみ閉じられる)
 */
export function PrivacyDialog({ children }: Props) {
  const [acknowledged, setAcknowledged] = useState<boolean | null>(null);
  const buttonRef = useRef<HTMLButtonElement>(null);

  // SSRではlocalStorage参照不可、マウント後に判定
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <p>
              この画面は <strong>架空データ専用</strong> です。
              実患者の医療情報を入力しないでください。
            </p>
            <ul className="list-disc space-y-1 pl-5 text-xs">
              <li>診断・治療・臨床判断には使用できません</li>
              <li>入力本文はサーバーログに記録されません</li>
              <li>抽出結果はサーバーに永続化されません (in-memoryのみ)</li>
              <li>
                {
                  '音声入力はブラウザ標準APIを使用しており、音声データはApple / Google等のクラウドに送信される場合があります'
                }
              </li>
              <li>ブラウザ側のキャッシュ / localStorageにはUI設定 (テーマ等) のみ保存されます</li>
            </ul>
