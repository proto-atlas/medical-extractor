...
/**
 * SOAPデータをJSON / CSV / Markdownにエクスポートするダウンロードボタン群。
 * ブラウザ標準のBlob + a[download] 経由でローカル保存させる (サーバー往復なし、
 * 入力本文をサーバーに送らないSPECのプライバシーポリシーと整合)。
 */
export function ExportButtons({ data, baseFilename = 'soap-extraction' }: Props) {
  function handleDownload(ext: 'json' | 'csv' | 'md', mimeType: string, content: string) {
    ...
    // 即座にrevokeするとFirefox等でダウンロード前にURLが無効化される報告があるため
    // 次フレームに遅延させる
