...
// APIレスポンスbodyの最小限型 (no-unsafe-member-access回避)
type ApiBody = {
  ok?: boolean;
  error?: string;
...
describe('/api/auth POST', () => {
  ...
  it('Authorizationヘッダなしなら 401 + ApiErrorResponse { error: unauthorized }', async () => {
    const res = await POST(makeReq({ ip: '203.0.113.22' }));
    expect(res.status).toBe(401);
    expect(await readBody(res)).toEqual({ error: 'unauthorized' });
  });

  it('5 回成功 + 6 回目は 429 + Retry-Afterヘッダ + retryAfterSeconds', async () => {
  ...
  it('rate limitは認証より先に評価される (誤パス連打でも 429 が先)', async () => {
