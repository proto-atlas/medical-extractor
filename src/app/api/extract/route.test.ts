...
// Anthropic SDKをモック (実APIを叩かない、テスト時の課金ゼロ)。
// `new Anthropic(...)` で呼ばれるためclass構文でmockしないと
...
// vi.mockのhoistingが確定してからrouteをimportする
const { POST, parseExtractRequest } = await import('./route');

// APIレスポンスbodyの最小限型定義 (テスト用)。
// no-unsafe-member-accessを回避するため、json() の戻り値をunknownではなくこの型で扱う。
type ApiBody = {
  error?: string;
...
describe('/api/extract POST', () => {
  beforeEach(() => {
    ...
    // 内部詳細露出抑止検証時にconsole.errorが呼ばれるのでテスト出力を汚さないよう抑止
    vi.spyOn(console, 'error').mockImplementation(() => {});
  });

  it('Authorization欠如なら 401 + error: unauthorized + Anthropicは呼ばれない', async () => {
  ...
  it('pre-auth limiter (extract-auth scope) が 11 回目で 429、Anthropicは呼ばれない', async () => {
    // /api/extractに不正Bearerを 10 回まで許す (extract-authは 10/60s)、
    // 11 回目でauth checkに到達する前に 429 を返す。Anthropic SDKは一切叩かない。
    const ip = '203.0.115.40';
    for (let i = 0; i < 10; i++) {
      const res = await POST(
        makeReq({ auth: 'Bearer wrong', ip, body: { documentText: '架空文書' } }),
      );
      // 認証は通らないがpre-authは通っているので 401 が返る
      expect(res.status).toBe(401);
    }
    const blocked = await POST(
      makeReq({ auth: 'Bearer wrong', ip, body: { documentText: '架空文書' } }),
    ...
    // pre-authで蹴られた + authで蹴られた両方の経路でAnthropicは呼ばれていない
    expect(mockCreate).not.toHaveBeenCalled();
  });

  it('extract-auth bucketとextract bucketは独立: pre-authに空きがあってもextractが枠切れなら 429', async () => {
    // 認証成功 5 回でextract scope上限到達。
    // pre-authは 5/10 消費なのでまだ余裕があるが、それでも本体側で 429。
  ...
  it('JSONが壊れていれば 400 + invalid_input', async () => {
    const res = await POST(
      makeReq({
        auth: 'Bearer correct-password',
        ip: '203.0.115.12',
  ...
  it('documentTextが空なら 400 + invalid_input', async () => {
    const res = await POST(
      makeReq({
        auth: 'Bearer correct-password',
        ip: '203.0.115.13',
  ...
  it('documentTextが 10001 文字なら 400 + document_too_long', async () => {
    const res = await POST(
      makeReq({
        auth: 'Bearer correct-password',
        ip: '203.0.115.14',
  ...
  it('ANTHROPIC_API_KEY不在なら 500 + server_misconfigured (環境変数名は露出しない)', async () => {
    ...
    // 環境変数名そのものがUI側に漏れていないことを確認
    expect(JSON.stringify(body)).not.toContain('ANTHROPIC_API_KEY');
  });

  it('SDKがtool_useを返さないなら 502 + tool_use_missing', async () => {
    mockCreate.mockResolvedValue({
  ...
  it('schema違反なら 502 + schema_violation (Zod issues詳細は露出しない)', async () => {
    mockCreate.mockResolvedValue({
      content: [
        {
          type: 'tool_use',
    ...
    // path / code等のZod詳細が漏れていないことを確認
    expect(body.schemaIssues).toBeUndefined();
    expect(body.issues).toBeUndefined();
  });

  it('source_textが入力本文に含まれないなら 502 + source_text_mismatch', async () => {
    mockCreate.mockResolvedValue({
      content: [
        {
          type: 'tool_use',
  ...
  it('SDK例外時は 500 + upstream_unavailable (生messageは露出しない)', async () => {
  ...
  it('正常系: tool_use + Zod passで 200 + soapデータ + model + usage', async () => {
    mockCreate.mockResolvedValue({
      content: [{ type: 'tool_use', name: 'extract_soap', input: validSoap }],
      usage: {
        input_tokens: 100,
  ...
  it('Anthropic timeout (APIConnectionTimeoutError相当) なら 504 + upstream_timeout', async () => {
    // 上流応答停滞時はupstream_unavailableから分離して
    // upstream_timeoutを返し、UIラベル「AIサービスの応答がタイムアウト
    // しました。」が出る経路を確保する。
  ...
  it('クライアント切断 (req.signal.aborted) なら 499、Anthropic upstream_unavailableに分類しない', async () => {
    // SDK呼び出しにsignalを渡したため、abort時はSDKが
    // AbortErrorをthrow → catchでreq.signal.abortedを見て 499 を返す。
    // 既存挙動を保つ確認テスト。
...
describe('parseExtractRequest (純関数)', () => {
  ...
  it('object以外 (string / number / array) はinvalid_input', () => {
    ...
    // 配列はobjectなのでフォーマット的には通るが、documentTextがstringでない => invalid_input
    expect(parseExtractRequest([])).toEqual({ ok: false, error: 'invalid_input' });
  });

  it('documentTextが文字列でないならinvalid_input', () => {
