...
describe('checkRateLimit (scope=extract, 5 req/60s)', () => {
  ...
  // medical-extractorのextractは 5 req/60s/IP (citation-readerの 10 より厳しい)。

  it('初回リクエストは許可され、remainingが 4 になる', () => {
  ...
  it('retryAfterSecondsは最低でも 1 を返す（切り上げ保証）', () => {
    const ip = '10.0.0.6';
    for (let i = 0; i < 5; i++) {
      checkRateLimit('extract', ip, 1000);
    }
...
describe('checkRateLimit (scope分離)', () => {
  // auth / extract / extract-authが独立bucketで管理されることを機械的に保証する。

  it('authとextractは独立bucket: 同IPでauthを 5 回消費してもextractは影響なし', () => {
    const ip = '10.0.1.1';
    for (let i = 0; i < 5; i++) {
  ...
  it('extractとextract-authは独立bucket: pre-auth連打してもextract本体には影響しない', () => {
    const ip = '10.0.1.2';
    for (let i = 0; i < 10; i++) {
      checkRateLimit('extract-auth', ip, 1000 + i);
    }
    // extract-authは 11 回目で 429
...
describe('checkRateLimit (scope=extract-auth, 10 req/60s)', () => {
  // /api/extractの認証前credential-attempt limiter。
  // extract / authより緩めの 10 req/60sで「正規UIの誤打吸収」「ボット遅延」を両立する。
...
describe('checkRequestRateLimit (Cloudflare binding)', () => {
  it('Cloudflare bindingがある場合はbinding名とscope/IP keyで評価する', async () => {
  ...
  it('Cloudflare bindingが許可した場合はedge cache limiterでも評価する', async () => {
  ...
  it('Cloudflare bindingがなければin-memory limiterにfallbackする', async () => {
    const result = await checkRequestRateLimit(
      'extract',
      '203.0.113.11',
      1000,
  ...
  it('Cloudflare bindingがない本番相当ではedge cache limiterで同一edge内のburstを止める', async () => {
