...
const validSOAP = {
  ...
  assessment: { text: 'う蝕C3相当、神経処置必要', source_text: 'う蝕C3 相当' },
...
describe('SOAPDataSchema', () => {
  ...
  it('未知のキーが混入していたら拒否する (.strict() / Anthropic strict tool useと整合)', () => {
    const result = SOAPDataSchema.safeParse({
      ...validSOAP,
  ...
  it('SOAPFieldレベルでも未知のキーは拒否される (.strict() nested)', () => {
    const result = SOAPDataSchema.safeParse({
      ...validSOAP,
  ...
  it('SOAP_TOOL_INPUT_SCHEMAはextract_soapという名前と整合しrequired 4 項目を持つ', () => {
  ...
  it('SOAP_TOOL_INPUT_SCHEMAの各フィールドはtext + source_textをrequiredにしている', () => {
  ...
  it('SOAP_TOOL_INPUT_SCHEMAはstrict tool use必須のadditionalProperties: falseを全レベルで持つ', () => {
    // トップ
...
describe('validateSOAPSourceTexts', () => {
  it('source_textが入力本文に含まれていればok: trueを返す', () => {
    const documentText =
      '右下奥歯の冷温水痛で来院。上顎 7 番に軽度の動揺を認める。う蝕C3 相当。次回根管治療予約。';

    expect(validateSOAPSourceTexts(documentText, validSOAP)).toEqual({ ok: true });
  });

  it('source_textの空白差分は正規化して照合する', () => {
    const documentText =
      '右下奥歯の冷温水痛で来院。上顎 7 番に軽度の動揺。う蝕C3 相当。次回根管治療予約。';
    const soap = {
      ...validSOAP,
  ...
  it('source_textの全角半角・記号・空白差分は正規化して照合する', () => {
    const documentText =
      '右下奥歯の冷温水痛で来院。上顎 7 番に軽度の動揺。う蝕C3 相当。次回根管治療予約 (1 週後)、抗生剤 (アモキシシリン 250mg) と鎮痛薬を 3 日分処方。';
    const soap = {
      ...validSOAP,
      plan: {
        text: '次回根管治療予約と処方',
  ...
  it('source_textが入力本文に含まれなければfield名だけを返す', () => {
    const documentText =
      '右下奥歯の冷温水痛で来院。上顎 7 番に軽度の動揺。う蝕C3 相当。次回根管治療予約。';
    const soap = {
      ...validSOAP,
  ...
  it('記載なし は欠落項目の明示マーカーとして許可する', () => {
    const documentText = '右下奥歯の冷温水痛で来院。上顎 7 番に軽度の動揺。う蝕C3 相当。';
    const soap = {
      ...validSOAP,
