...
- Vitest 4.1.5 (ユニット 125、coverage stmts 93.42 / branches 86.11 / funcs 100 / lines 96.32) + happy-dom + Playwright 1.59 (E2E 17 シナリオ Chromium = auth 4 + privacy 3 + extract 3 + axe a11y 4 + target-size 3、Firefox / WebKit / Mobile は CI matrix 外)
...
```bash
npm run check # typecheck + lint + Vitest (125)
npm run test:coverage # coverage gate (lines 60 / functions 70 / branches 50 / statements 60)
npx playwright test --project=chromium # E2E Chromium (17 シナリオ: auth/privacy/extract/a11y/target-size)
...
| 項目 | コマンド | 結果 / Evidence |
|---|---|---|
| 静的解析 + ユニット + coverage gate + build | `npm run check` / `npm run test:coverage` / `npm run build` | typecheck pass / lint pass / Vitest 14 files 125 件 / stmts 93.42 branches 86.11 funcs 100 lines 96.32 / build pass |
| Chromium E2E (auth / privacy / extract / a11y / target-size 17 件) | `npm run e2e -- --project=chromium` | 全 pass |
