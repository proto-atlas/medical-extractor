...
/* eslint-env node */
/**
 * SOAP抽出の構造的正確性を実Anthropic APIで評価するスクリプト。
 *
 * 使い方:
 * npm run eval:soap # dry run (fixture読み込みと構造のみ表示、API課金ゼロ)
 ...
 *
 * 安全装置:
 * - RUN_LIVE_ANTHROPIC=1 が無ければ実送信しない (デフォルトdry run)
 * - ANTHROPIC_API_KEYが無ければ即終了
 * - --limitは 1〜3 にclamp (架空データ 3 件以上送らない、コスト保護)
 *
 * 評価項目:
 * - subjective / objective / assessment / planの 4 項目存在
 * - 各項目のtext / source_text非空
 * - source_textが原文 (documentText) に部分一致
 * - shouldContainキーワードがtextまたはsource_textに含まれる
 * - tool schema strict (additionalProperties: false) 準拠 = Anthropic側で保証
 */
...
// SOAP tool schema (src/lib/soap-schema.tsと整合させて維持。
// schemaを変更する場合は両方を同時に更新すること)。
const SOAP_TOOL_NAME = 'extract_soap';
const SOAP_TOOL_DESCRIPTION =
  '医療文書からSOAP形式 (Subjective / Objective / Assessment / Plan) を構造化抽出するツール。各項目にtext (整理サマリー) とsource_text (原文の該当部分の引用) を含める。';
const SOAP_FIELD = {
  type: 'object',
  properties: {
...
function structuralChecks(soap, fixture) {
  ...
  for (const s of SECTIONS) {
    ...
    checks.push({ name: `${s}.text非空`, pass: text.length > 0 });
    checks.push({ name: `${s}.source_text非空`, pass: src.length > 0 });

    if (src.length > 0) {
      const inDoc = sourceTextMatchesDocument(src, fixture.documentText);
      checks.push({
        name: `${s}.source_text原文一致`,
        pass: inDoc,
        detail: inDoc
          ? undefined
          : `"${src.slice(0, 40)}..." が空白正規化後もdocumentTextに含まれない`,
      });
    }
    ...
    for (const kw of expected) {
      ...
      checks.push({
        ...
        detail: pool.includes(kw) ? undefined : `text/source_textにキーワード "${kw}" 不在`,
      });
    }
  }
...
async function dryRun(fixtures, limit) {
  console.log('[dry-run] RUN_LIVE_ANTHROPIC != 1 のため実APIは叩きません');
  console.log(`fixture件数: ${fixtures.length}, --limit=${limit} (clamped 1〜${MAX_LIMIT})`);
  console.log('');
  ...
  console.log('');
  console.log('実API評価を実行するには:');
  console.log(' RUN_LIVE_ANTHROPIC=1 ANTHROPIC_API_KEY=sk-... npm run eval:soap');
  console.log(' オプション: -- --limit=N (1〜3、デフォルト 1)');
}

/**
 * fixture.domainNotes (FHIR Resource候補 + ICD-10/SNOMED範囲外宣言 + scope) を
 * 標準出力に表示する:
 * docsだけでなくeval scriptの出力にもmedical-domain境界をmentionし、
...
function printDomainNotes(fixture) {
  ...
  console.log(` ICD-10 / SNOMED CT確定: ${dn.icd10NotApplicable ? 'out-of-scope' : 'in-scope'}`);
  console.log(` 診断支援: ${dn.diagnosticAdvice === false ? 'out-of-scope' : 'unknown'}`);
}

async function liveRun(fixtures, limit) {
  // dynamic importでdry run時の余計な依存ロードを避ける
...
function buildEvidenceMarkdown(mode, fixtures, limit, results) {
  ...
  lines.push('## 安全装置');
  lines.push('');
  lines.push('- `RUN_LIVE_ANTHROPIC=1` 必須 (無ければdry run)');
  lines.push('- `ANTHROPIC_API_KEY` 必須');
  lines.push('- `--limit` は 1〜3 にclamp (架空データのみ、コスト保護)');
  lines.push('- `source_text` 照合は、原文の改行差分だけを空白正規化して判定');
  lines.push('');
  lines.push('## fixture一覧');
  lines.push('');
  lines.push('| id | specialty | 文字数 | 期待section |');
  lines.push('|---|---|---:|---|');
  ...
  lines.push('');
  lines.push(
    '本evalは **構造化抽出品質** を評価する。FHIR R5 への変換、ICD-10 / SNOMED CTコード付与、診断支援、治療推奨は **範囲外**。',
  );
  lines.push('詳細は [`docs/medical-domain-evidence.md`](../medical-domain-evidence.md) を参照。');
  lines.push('');
  lines.push('| fixture | scope | ICD-10 / SNOMED確定 | 診断支援 |');
  lines.push('|---|---|---|---|');
  for (const fx of fixtures) {
  ...
  if (mode === 'dry-run') {
    lines.push('## 結果');
    lines.push('');
    lines.push('実API未送信。fixtureの構造検証のみ実施。');
    lines.push('');
    lines.push('実評価を行うには:');
    lines.push('');
    lines.push('```bash');
  ...
  if (rowsWithSoap.length > 0) {
    ...
    lines.push('');
    lines.push(
      'Anthropic `tool_use` の `input_schema` (`strict: true`, `additionalProperties: false`) を通った構造化出力。fixtureは架空データのみ。',
    );
    lines.push('');
...
async function main() {
  ...
  if (fixtures.length === 0) {
    console.error(`fixtureが見つかりません: ${FIXTURE_DIR}`);
    process.exit(1);
  }
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error('ANTHROPIC_API_KEYが未設定です。実API評価を中止します。');
    process.exit(1);
  }
