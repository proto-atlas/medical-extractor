 ...
 * 本番URLに対する 疎通確認 テスト。
 *
 * 本番確認チェックリストをscript化し、公開URL確認evidenceを生成する。
 *
 * 使い方:
 * PRODUCTION_URL=https://medical-extractor.atlas-lab.workers.dev node scripts/check-production.mjs
 *
 * 出力:
 * docs/evidence/production-check-{ISO_DATE}.md (人間用サマリ)
 * docs/evidence/production-check-{ISO_DATE}.json (生result)
 *
 * 安全装置:
 * - PRODUCTION_URL必須 (デフォルトなし、誤ってlocalhostに投げない)
 * - rate-limitを消費する 6/11 連打は別フラグ (`--burst-rate-limit`) でのみ実行
 * (運用中の影響を避けるため、デフォルトはstatus / headers検査のみ)
 * - Anthropic課金経路 (/api/extractの正規Bearer + body) は実行しない
 */
...
const PRIOR_UNAUTHORIZED_CHECK_REQUESTS = 1;
const AUTH_EXPECTED_FIRST_429_ATTEMPT = AUTH_LIMIT - PRIOR_UNAUTHORIZED_CHECK_REQUESTS + 1;
const EXTRACT_AUTH_EXPECTED_FIRST_429_ATTEMPT =
  EXTRACT_AUTH_LIMIT - PRIOR_UNAUTHORIZED_CHECK_REQUESTS + 1;
...
if (!PRODUCTION_URL) {
  console.error('PRODUCTION_URL環境変数が必須です');
  console.error(
    '例: PRODUCTION_URL=https://medical-extractor.atlas-lab.workers.dev node scripts/check-production.mjs',
  );
  process.exit(1);
}
...
async function checkAuthRateLimit() {
  // 直前のno-auth疎通確認 がauth bucketを 1 回消費するため、
  // burst内では 5 回目に 429 を期待 (auth scopeは 5 req/60s)。
  const url = `${base}/api/auth`;
  const responses = [];
  for (let i = 0; i < 6; i++) {
    const r = await fetchSafe(url, {
...
async function checkExtractPreAuthRateLimit() {
  // 直前のno-auth疎通確認 がextract-auth bucketを 1 回消費するため、
  // burst内では 10 回目に 429 を期待 (extract-auth scopeは 10 req/60s)。
  // Anthropic課金は発生しない (auth check前に弾かれる)
  const url = `${base}/api/extract`;
  const responses = [];
  for (let i = 0; i < 11; i++) {
    const r = await fetchSafe(url, {
...
function buildMarkdown(result) {
  const lines = [];
  lines.push('# 公開URL確認結果');
  lines.push('');
  lines.push(`実施日: ${ISO_DATE}`);
  lines.push(`対象URL: \`${base}\``);
  lines.push(
    `burst rate-limit検証: ${BURST_RATE_LIMIT ? '実施' : 'スキップ (--burst-rate-limitで有効化)'}`,
  );
  lines.push('');
  lines.push('## 静的ルート');
  lines.push('');
  ...
  lines.push('');
  lines.push('## APIルート (Anthropic課金なし経路)');
  lines.push('');
  lines.push('| URL | 期待 | 実測 | pass | error |');
  lines.push('|---|---:|---:|---|---|');
  lines.push(
    `| ${result.authUnauth.url} (Authorizationなし) | 401 | ${result.authUnauth.status ?? '?'} | ${result.authUnauth.pass ? '✓' : '✗'} | ${JSON.stringify(result.authUnauth.body) ?? '-'} |`,
  );
  lines.push(
    `| ${result.extractUnauth.url} (Authorizationなし) | 401 | ${result.extractUnauth.status ?? '?'} | ${result.extractUnauth.pass ? '✓' : '✗'} | ${JSON.stringify(result.extractUnauth.body) ?? '-'} |`,
  );
  lines.push('');
  if (BURST_RATE_LIMIT) {
    lines.push('## Rate-Limit Burst検証');
    lines.push('');
    lines.push(
    ...
    lines.push('');
    lines.push('### extract-auth scope (10 req/60s, 認証前limiter)');
    lines.push('');
    lines.push('| attempt | status | retry-after | body |');
    lines.push('|---:|---:|---|---|');
  ...
  lines.push(allPass ? '✓ all pass' : '✗ 1 件以上fail');
  return lines.join('\n');
}

async function main() {
  console.log(`公開URL確認target: ${base}`);
  console.log(
  ...
  if (BURST_RATE_LIMIT) {
    console.log('');
    console.log(' [burst] /api/auth × 6 (通常401 疎通確認で1回消費後、burst 5回目に429を期待)');
    authBurst = await checkAuthRateLimit();
    console.log(` → ${authBurst.pass ? `✓ ${authBurst.first429Attempt} 件目で 429` : '✗ fail'}`);
    console.log(
      ' [burst] /api/extract × 11 (通常401 疎通確認で1回消費後、burst 10回目に429を期待)',
    );
  ...
  const jsonPath = join(EVIDENCE_DIR, `production-check-${ISO_DATE}.json`);
  await writeFile(jsonPath, `${JSON.stringify(result, null, 2)}\n`, 'utf-8');
  const mdPath = join(EVIDENCE_DIR, `production-check-${ISO_DATE}.md`);
